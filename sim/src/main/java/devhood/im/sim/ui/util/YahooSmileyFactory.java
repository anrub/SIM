...
public class YahooSmileyFactory implements SmileyFactory {
	...
	/**
	 * Default-Konstruktor initialisiert die Map.
	 */
	public YahooSmileyFactory() {
		init();
	}
	 ...
	 * 
	 * @param String text Der Text in dem die Smiley's ausgetausch werden sollen.
	 */
	public String applySmiles(String text) {
		...
		for (String[] keys : smileys.keySet()){
			for (String key : keys){
				if (text.equals(key)) {
					return "<img src=\"" + getClass().getResource(imagePath + smileys.get(keys)).toString() + " \" />";
				}
			}
		}
	...
	/**
	 * Die Initialisierung führt das Pairing der Kurzschreibweisen zu den Bildnamen durch.
	 */
	public void init(){
		...
		smileys.put(new String[]{":-p",":-P",";-p",";-P",":p",":P",";p",";P",":-)p",":-)P",";-)p",";-)P",":)p",":)P",";)p",";)P"}, "09.gif");
		...
		smileys.put(new String[]{":-bd",":-)bd",":)bd","b:)d","b:-)d"}, "113.gif");
